...
| `user.role` | string | Rôle dans l'organisation (ex. `admin`). |
| `operator` | object | Profil opérateur associé à l'utilisateur. |
...
<Aside type="note">
  `user.role` est actuellement `"admin"` pour tout nouvel inscrit (premier administrateur de l'organisation). Ce champ peut être vide si la relation organisation/opérateur n'est pas encore créée.
</Aside>
