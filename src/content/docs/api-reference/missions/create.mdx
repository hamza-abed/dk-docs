...
| `custom_fields` | object | Non | Champs personnalisés (JSON libre). |
...
<Aside type="note">
  Si `operator_id` est fourni, la mission est créée avec le statut `assigned` et l'opérateur reçoit immédiatement une notification push. Sinon, le statut est `unassigned`.
</Aside>
