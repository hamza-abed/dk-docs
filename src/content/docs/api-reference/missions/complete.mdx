...
| `latitude` | string | Non | Latitude de la position au moment de la complétion (ex: `"48.8698"`). Envoyé comme string en multipart, parsé en float côté backend. |
| `longitude` | string | Non | Longitude de la position au moment de la complétion (ex: `"2.3311"`). Envoyé comme string en multipart, parsé en float côté backend. |
| `photo` | file | Non | Photo de preuve / reçu (upload fichier). |

### Exemple de requête

```bash
curl -X POST https://api.dispatchkit.io/api/v1/missions/c3d4e5f6-a7b8-4012-8def-123456789012/complete \
...
```json
{
  ...
  "receipt": {
    ...
    "photo_url": "receipts/c3d4e5f6-a7b8-4012-8def-123456789012/receipt-abc123.jpg"
  },
