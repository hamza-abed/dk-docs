...
<ParamField path="id" type="string" required>
  Identifiant de la mission à compléter. Doit être au statut `IN_PROGRESS` ou `ON_SITE`.
</ParamField>

### Body

<ParamField body="notes" type="string">
  Notes de complétion de l'opérateur (observations, incidents, etc.). Max 2000 caractères.
</ParamField>

<ParamField body="photos" type="array">
  URLs des photos de preuve uploadées via l'API d'upload. Max 5 photos.
</ParamField>

<ParamField body="signature_url" type="string">
  URL de l'image de signature du client, uploadée via l'API d'upload.
</ParamField>
