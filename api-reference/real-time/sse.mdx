...
<ParamField header="Authorization" type="string">
  `Bearer {token}` — Alternative au query param `token`.
</ParamField>

<ParamField header="Accept" type="string">
  Doit être `text/event-stream`.
</ParamField>

### Query params

<ParamField query="token" type="string">
  Token JWT en alternative au header Authorization (utile avec `EventSource` navigateur qui ne supporte pas les headers custom).
</ParamField>
